...
class Agent:
    ...
    # Saving the state dictionary of the network and other parameters
        state = {
            "online_network_state": self.online_network.state_dict(),
            "epsilon": self.epsilon,
            "learn_step_counter": self.learn_step_counter,
            "eps_decay": self.eps_decay,
           # Add other relevant agent states if necessary
    ...
    def load_model(self, path):
        ...
        # Load 'learn_step_counter' if it exists, otherwise default to 0
        self.learn_step_counter = state.get("learn_step_counter", 0)

        # Restoring other agent parameters
        self.epsilon = state.get("epsilon", 1.0)  # Default to 1.0 if 'epsilon' key is missing
        self.eps_decay = state.get("eps_decay", 0.99999975)  # Set default value if 'eps_decay' is missing
        # Add similar lines for other parameters if necessary
